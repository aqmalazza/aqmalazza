# Hi there, I'm M.A Madani 👋

I'm a Full-Stack Developer who still has a lot to learn, but that's exactly what makes the journey exciting.  
Currently, I'm studying at **Nusantara Islamic University** while building, exploring, and improving my skills one project at a time.

If you want to learn, build, or grow together, feel free to reach out. Let's connect and make something fun 🚀

---

## About Me

- 💻 Full-Stack Developer in progress
- 🎨 UI/UX design enthusiast
- 🖌️ Graphic design enjoyer
- 🧠 Always learning, sometimes confused, but never stopping
- ☕ Turning ideas, coffee, and curiosity into web projects
- 🤝 Open to collaboration, learning partners, and fun tech discussions

---

## Tech Stack

### Languages

[![Languages](https://skillicons.dev/icons?i=js,cpp,py,php)](https://skillicons.dev)

![SWI-Prolog](https://img.shields.io/badge/SWI--Prolog-8B0000?style=flat&logo=prolog&logoColor=white)

### Frontend

[![Frontend](https://skillicons.dev/icons?i=html,css,react,tailwind,vite)](https://skillicons.dev)

### Backend

[![Backend](https://skillicons.dev/icons?i=nodejs,express)](https://skillicons.dev)

### Database

[![Database](https://skillicons.dev/icons?i=mysql,postgres)](https://skillicons.dev)

### Development Tools

[![Development Tools](https://skillicons.dev/icons?i=git,github,vscode)](https://skillicons.dev)

### Design Tools

[![Design Tools](https://skillicons.dev/icons?i=figma,ps)](https://skillicons.dev)

---

## Currently Exploring

- Building better full-stack applications
- Improving UI/UX and visual design skills
- Writing cleaner and more maintainable code
- Learning how to turn ideas into real digital products

---

## Connect With Me

- GitHub: [@aqmlmdn](https://github.com/aqmlmdn)
- Email: Cooming Soon
- Linkedin : Cooming Soon

---

> Still learning, still building, and still enjoying the process.
> But remember, theory without action is just a fantasy.
